...
| Full-suite scheduler | unchanged default pytest selection with exact process-isolated work stealing and serial rollback | `scripts/run_pytest_lane.py` | tests, release |
...
| Agent skill projection | transitional repo-local `.agents/skills` projection, sibling `aoa-skills` symlink targets, and checkout-safe target files | `scripts/validators/agent_skill_projection.py` | source-fast, release |
...
| Federation runtime seams | memo/eval/playbook/KAG runtime seam docs, C20 content-minimized active-organ delivery receipts, executable positive/negative examples, bounded export schemas/examples, advisory route guards | `scripts/validators/federation_runtime_seams.py` | source-fast, release |
| Diagnostic spine contracts | diagnostic surface docs, schemas, examples, catalog refs, and canonical owner skill package | `scripts/validators/diagnostic_spine.py` | source-fast, release |
...
| Agent route topology | nested `AGENTS.md` coverage, explicit legacy-archive card classification, route snippets, and the 32-KiB inherited-chain budget | `scripts/validate_nested_agents.py` | source-fast, release |
...
| `scripts/run_pytest_lane.py` | bounded full-suite scheduler | fix the baseline/partition/observed-selection proof or use the explicit serial rollback; never remove a failing test from selection |
...
| `scripts/validators/agent_skill_projection.py` | agent skill projection validator module | fix the transitional `.agents/skills` symlink/target-file projection or focused agent-skill tests |
...
| `scripts/validators/federation_runtime_seams.py` | federation runtime seam validator module | fix memo/eval/playbook/KAG seam docs, C20 delivery receipt contracts/examples, bounded export examples, A2A handoff examples, or focused federation runtime seam tests |
| `scripts/validators/diagnostic_spine.py` | diagnostic spine contract validator module | fix diagnostic docs, schemas, examples, generated catalog refs, canonical owner skill package, or focused diagnostic spine tests |
...
| `scripts/validate_nested_agents.py` | AGENTS route topology | fix the nearest active `AGENTS.md`, repeated parent guidance, required route doc, inherited-chain budget, or explicit legacy-archive classification |
...
The `decision-graph` lane refreshes its ignored local cache before checking it.
This keeps the landing gate self-contained in a clean checkout and preserves
`--check` as a deterministic parity assertion rather than an implicit writer.
