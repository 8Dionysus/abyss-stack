...
It reads a mirrored `aoa-agents` contract seam, an `aoa-routing advisory seam`, an `aoa-memo` recall seam, an `aoa-evals` eval selection seam, and an `aoa-playbooks` activation/composition advisory seam through the single localhost-only `route-api`.
