...
- A distinct, inert `aoa-routing-cutover` path now admits only a
  ...
  recovered from exact on-disk state. Corrupt trust collections fail closed
  without crashing health. The path neither performs the owner switch when
  merged nor authorizes predecessor archival.
