...
same exact manual units transiently when the production fleet cannot start and
its still-current admission is not reusable for the exact deployment, or when
the registry source or admitted read currentness has expired. A deployment,
package, production-canary, or managed-catalog identity mismatch therefore
rebuilds evidence instead of retrying a preflight that must fail closed. It
first resets the expired registry to claim-free shadow state when required,
admits only bootstrap-bound evidence long enough to rebuild an 11-of-11 eligible
...
the shared runtime-provision lock, and binds the signed canary's exact process
unit to its production, bootstrap, or fallback template. Intermediate preflight
therefore verifies the template actually serving the recovery canary; final
preflight independently verifies production, without accepting a stale sibling
template or an unserialized reader command.
