...
- `aoa_session_literal_query_plan(query="", kind="auto", filters)`; plans the cheapest reliable route before broad literal raw-text search. It prefers entity usage/trace/graph for typed operational anchors, entity registry/inventory for broad class queries such as skills/MCP/hooks/tools, structured filters for exact route reads, scoped full-text shards when available, and monolith fallback only as a bounded recall safety net.
