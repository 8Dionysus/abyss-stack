"""Read-only MCP access plane for the Telegram connector."""
...
@dataclass
class AoATelegramConnectorMCPState:
    """State and command bridge for the Telegram connector MCP service."""
    ...
    def discover(cls) -> "AoATelegramConnectorMCPState":
        ...
        return cls(
            ...
            default_run=os.environ.get("AOA_TELEGRAM_CONNECTOR_RUN", DEFAULT_RUN),
            timeout=_env_float("AOA_TELEGRAM_CONNECTOR_MCP_TIMEOUT", DEFAULT_TIMEOUT_SECONDS),
        )
    ...
    def source_route(self) -> dict[str, Any]:
        return {
            "schema": "aoa_telegram_connector_mcp_source_route_v1",
            ...
            "permission_modes": ["bot_api", "tdlib_user_session", "takeout_export"],
            ...
            "env_vars": {
                "connector_repo": "AOA_TELEGRAM_CONNECTOR_REPO",
                "connector_bin": "AOA_TELEGRAM_CONNECTOR_BIN",
                "default_run": "AOA_TELEGRAM_CONNECTOR_RUN",
                "timeout": "AOA_TELEGRAM_CONNECTOR_MCP_TIMEOUT",
    ...
    def status(self) -> dict[str, Any]:
        ...
        return {
            "schema": "aoa_telegram_connector_mcp_status_v1",
            "service_name": SERVICE_NAME,
    ...
    def answer(self, query: str, *, run: str | None = None, limit: int = 5) -> dict[str, Any]:
        ...
        if payload.get("schema") not in {"aoa_connector_answer_packet_v1", "aoa_telegram_answer_packet_v1"}:
            boundary_errors.append("connector packet schema is not a known answer packet schema")
        return {
            "schema": "aoa_telegram_connector_mcp_answer_v1",
            "service_name": SERVICE_NAME,
    ...
    def query_graph(self, query: str, *, run: str | None = None, limit: int = 5) -> dict[str, Any]:
        ...
        payload = result.payload if isinstance(result.payload, dict) else {}
        return {
            "schema": "aoa_telegram_connector_mcp_query_graph_v1",
            "service_name": SERVICE_NAME,
    ...
    def read_resource(self, uri: str) -> dict[str, Any]:
        if uri == "aoa-telegram://source-route":
            return self.source_route()
        if uri == "aoa-telegram://status":
            return self.status()
        raise KeyError(f"unknown aoa-telegram connector MCP resource URI: {uri}")
