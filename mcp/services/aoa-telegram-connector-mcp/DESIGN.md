# Telegram Connector MCP Design

## Purpose

Expose already-built local Telegram connector evidence to agents through MCP
while preserving source ownership in `aoa-telegram-connector`.

## Shape

The service is a thin adapter over either:

- an installed `aoa-telegram` executable, or
- a local `AOA_TELEGRAM_CONNECTOR_REPO` checkout.
...
The HTTP lifecycle uses an exact Telegram read credential and a read-only,
