...
- **external model incarnations**: a post-compile, owner-bound Codex process
  ...
  thread/re-entry evidence, projects a typed incarnation home instead of
  inheriting ambient operator state, and feeds independently reviewed returns
  back into governed execution without using built-in subagent transport
