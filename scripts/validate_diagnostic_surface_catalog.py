...
def main() -> int:
    ...
    if current_payload != expected_payload:
        raise SystemExit("generated/diagnostic_surface_catalog.min.json does not match the canonical rebuild")

    for key, expected in SURFACE_PAYLOAD.items():
        if current_payload.get(key) != expected:
            raise SystemExit(
                f"generated/diagnostic_surface_catalog.min.json must keep {key}={expected!r}"
            )
    ...
    if not isinstance(surfaces, list) or len(surfaces) != len(SURFACE_SPECS):
        raise SystemExit("generated/diagnostic_surface_catalog.min.json must publish exactly five diagnostic surfaces")
    for surface, spec in zip(surfaces, SURFACE_SPECS, strict=True):
        if not isinstance(surface, dict):
            raise SystemExit("generated/diagnostic_surface_catalog.min.json surfaces must be objects")
        for key in ("name", "schema_ref", "example_ref", "primary_question"):
            if surface.get(key) != spec[key]:
                raise SystemExit(
                    f"generated/diagnostic_surface_catalog.min.json surface '{spec['name']}' must keep {key}"
                )
    ...
    if validation_refs != VALIDATION_REFS:
        raise SystemExit("generated/diagnostic_surface_catalog.min.json validation_refs drifted")
    for ref in VALIDATION_REFS:
        resolve_ref(ref)

    print("[ok] validated generated/diagnostic_surface_catalog.min.json")
    return 0
