from __future__ import annotations

import ast
from pathlib import Path
import tomllib


def _uses_ambient_distribution_metadata(tree: ast.Module) -> bool:
    for node in ast.walk(tree):
        if isinstance(node, ast.ImportFrom) and node.module == "importlib.metadata":
            return True
        if isinstance(node, ast.Import):
            if any(alias.name == "importlib.metadata" for alias in node.names):
                return True
    return False


def _modern_server_calls(tree: ast.Module) -> list[ast.Call]:
    for node in tree.body:
        if not isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            continue
        if node.name != "build_server":
            continue
        return [
            descendant
            for descendant in ast.walk(node)
            if isinstance(descendant, ast.Call)
            and isinstance(descendant.func, ast.Name)
            and descendant.func.id == "ModernMCPServer"
        ]
    return []


def _is_service_version_reference(node: ast.AST) -> bool:
    return (
        isinstance(node, ast.Attribute)
        and node.attr == "package_version"
        and isinstance(node.value, ast.Name)
        and node.value.id == "SERVICE_CONFIG"
    )


def validate(errors: list[str], *, root: Path) -> None:
    services_root = root / "mcp" / "services"
    for pyproject_path in sorted(services_root.glob("*/pyproject.toml")):
        package_root = pyproject_path.parent
        relative_package = package_root.relative_to(root)
        try:
            project = tomllib.loads(pyproject_path.read_text(encoding="utf-8"))[
                "project"
            ]
            project_version = project["version"]
        except (KeyError, tomllib.TOMLDecodeError) as exc:
            errors.append(
                f"{relative_package}/pyproject.toml has no readable project.version: {exc}"
            )
            continue

        server_paths = sorted((package_root / "src").glob("*/server.py"))
        if len(server_paths) != 1:
            errors.append(
                f"{relative_package} must expose exactly one src/*/server.py "
                "for MCP application identity"
            )
            continue

        server_path = server_paths[0]
        relative_server = server_path.relative_to(root)
        try:
            tree = ast.parse(server_path.read_text(encoding="utf-8"))
        except SyntaxError as exc:
            errors.append(f"{relative_server} cannot be parsed: {exc}")
            continue

        if _uses_ambient_distribution_metadata(tree):
            errors.append(
                f"{relative_server} must not derive serverInfo.version from "
                "ambient importlib.metadata"
            )
        modern_server_calls = _modern_server_calls(tree)
        if not modern_server_calls:
            errors.append(
                f"{relative_server} build_server must construct native ModernMCPServer"
            )
            continue
        version_bindings = [
            keyword.value
            for call in modern_server_calls
            for keyword in call.keywords
            if keyword.arg == "version"
        ]
        if len(version_bindings) != len(modern_server_calls) or any(
            not _is_service_version_reference(value) for value in version_bindings
        ):
            errors.append(
                f"{relative_server} ModernMCPServer version must be bound from "
                "SERVICE_CONFIG.package_version"
            )
        if not project_version:
            errors.append(
                f"{relative_server} cannot validate its package version from pyproject.toml"
            )
