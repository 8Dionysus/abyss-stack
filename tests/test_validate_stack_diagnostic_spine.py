...
class ValidateStackDiagnosticSpineTests(unittest.TestCase):
    def write_valid_surface(self, repo_root: Path) -> None:
        for relative_path in (
            ...
            Path("schemas") / "diagnostic_anchor_ref.schema.json",
            Path("schemas") / "repair_handoff.schema.json",
            Path("schemas") / "reviewed_diagnosis_ref.schema.json",
            Path("examples") / "diagnostic_target.min.example.json",
            Path("examples") / "diagnostic_session.min.example.json",
            Path("examples") / "diagnosis_companion.min.example.json",
            Path("examples") / "diagnostic_anchor_ref.min.example.json",
            Path("examples") / "repair_handoff.min.example.json",
            Path("examples") / "reviewed_diagnosis_ref.min.example.json",
        ):
            write_text(
    ...
    def test_session_example_exit_class_drift_fails(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.write_valid_surface(repo_root)
            write_text(
                repo_root / "examples" / "diagnostic_session.min.example.json",
                (repo_root / "examples" / "diagnostic_session.min.example.json")
