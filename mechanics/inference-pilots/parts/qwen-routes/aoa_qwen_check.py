...
REPO_CHOICE_EXPECTED = "aoa-evals"
JSON_DECISION_EXPECTED = {
    "verdict": "candidate",
    "repo": "abyss-stack",
}
...
"""
    ...
    if case == "repo-choice":
        prompt = "Return one exact repo name only. Allowed answers: aoa-evals, abyss-stack, Tree-of-Sophia. portable proof surfaces for bounded claims."
        return prompt, 8

    if case == "json-decision":
        prompt = 'Return compact JSON {"verdict":"...","repo":"..."}. Use exact tokens only. verdict = candidate. repo = abyss-stack.'
        return prompt, 32
...
def run_check(url: str, case: str, timeout_s: float, temperature: float, max_tokens: int | None) -> dict[str, object]:
    ...
    elif case == "repo-choice":
        validation["expected"] = REPO_CHOICE_EXPECTED
        validation["observed"] = answer
        ok = answer == REPO_CHOICE_EXPECTED
    elif case == "json-decision":
        parsed = json.loads(extract_json_block(answer))
        validation["expected"] = JSON_DECISION_EXPECTED
        validation["observed"] = parsed
        ok = parsed == JSON_DECISION_EXPECTED
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--case",
        choices=["exact-reply", "repo-routing", "repo-choice", "json-decision"],
        required=True,
    )
    parser.add_argument("--url", default="http://127.0.0.1:5403/run")
