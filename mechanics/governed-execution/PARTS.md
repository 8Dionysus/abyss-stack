...
| External Codex agent | `parts/external-codex-agent/` | `scripts/aoa-external-actor-bind`, `scripts/aoa-external-codex-agent`, role-first owner admission with pinned `aoa-agents`/`aoa-skills` schemas, content-addressed machine-local runtime installer/status/rollback, exact launch/task/profile/state/event/report/result/study/review-preparation schemas, separate Codex process, role-scoped MCPs, observe-only usage metering, durable thread resume, canonical independent-review preparation and A2A-return export |
