...
| External Codex agent | `parts/external-codex-agent/` | `scripts/aoa-external-codex-agent`, content-addressed machine-local runtime installer/status/rollback, exact launch/task/profile/state/event/report/result/study/review-preparation schemas, separate Codex process, observe-only usage metering, durable thread resume, canonical independent-review preparation and A2A-return export |
