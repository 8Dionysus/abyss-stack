...
holder writes a non-replacing lifecycle receipt before `exec`, records its
process parent and first Kitty ancestor, rejects receipt binding for detached
Kitty launches, and keeps holder identity separate from runtime proof-actor
identity. The installed `close` route is only valid after the exact wake
receipt proves handoff delivery; it rechecks PID/start-ticks, argv, process
parent, and the Kitty ancestor before sending `TERM`. A live visible trial must
separately prove the installed release, delivery, holder disappearance,
