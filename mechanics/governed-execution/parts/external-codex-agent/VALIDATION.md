...
`paused`, the returned identity and status are required before the receipt is
 ...
 A companion case proves that a paused observation without that marker fails
 closed. The fixture asserts that pause does not use turn delivery, terminal
 input, process identity, holder closure, or wake evidence.
