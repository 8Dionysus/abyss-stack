...
- adoption, retention, rollback, release, watchtower, canary, and deployment
  records
- governance, constitution, veto, stay, appeal, and council records
- federation harvest, KAG promotion, ToS dossier, and pattern registry records
...
Promote one slice only when all of these are true:

- one active `abyss-stack` service, storage path, or operator route consumes it
- the active name does not preserve old wave, seed, or version scaffolding as
  topology
...
`Agents-of-Abyss` owns experience program meaning, governance doctrine, release
approval, adoption meaning, and council-style authority.

`Tree-of-Sophia` owns authored meaning and direct-write stop-lines.

`abyss-stack` may own only runtime transport, validation, storage shape,
operator routes, and public-safe examples after a concrete consumer exists.
