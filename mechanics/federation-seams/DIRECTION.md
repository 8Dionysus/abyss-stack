...
- keep RPG, repo-self KAG, and ToS graph projections as source-derived runtime
  read models
- keep repo-self KAG exact rows, vector owner collections, and graph
  owner/owner-pair slices independently replaceable while binding their
  current and last-good maps to one projection generation
...
- keep ordinary repo-self owner changes local to affected exact, vector, and
  graph slices while central semantic changes retain the full-bootstrap route
- keep repo-self exact, vector, and graph rollback bound to one verified
  `aoa-kag` bundle, projection, and federation identity
