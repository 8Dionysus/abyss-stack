...
def test_build_count_and_digest():
    mod = load('builder', PART_ROOT / 'build_mechanical_trial_run_registry.py')
    reg = mod.build()
    assert reg['count'] == 7
    assert reg['runtime_posture'] == 'local_dry_run_candidate_only'

def test_validator_green():
    mod = load('validator', PART_ROOT / 'validate_mechanical_trial_runs.py')
    assert mod.main() == 0

def test_validator_rejects_tampered_event_hash(tmp_path):
    mod = load('validator', PART_ROOT / 'validate_mechanical_trial_runs.py')
    source = json.loads((EXAMPLES / 'mechanical-trial-event-log.prediction.example.json').read_text(encoding='utf-8'))
...
def test_validator_rejects_stale_generated_registry(tmp_path):
    validator = load('validator', PART_ROOT / 'validate_mechanical_trial_runs.py')
    builder = load('builder', PART_ROOT / 'build_mechanical_trial_run_registry.py')
    stale = builder.build()
    stale['digest'] = '0' * 64
    out = tmp_path / 'mechanical-trial-run-registry.min.json'
    out.write_text(json.dumps(stale), encoding='utf-8')
    validator.OUT = out
    assert validator.main() == 1

def test_simulator_check_green():
    result = subprocess.run([sys.executable, str(PART_ROOT / 'simulate_mechanical_trials.py'), '--check'], cwd=str(ROOT))
