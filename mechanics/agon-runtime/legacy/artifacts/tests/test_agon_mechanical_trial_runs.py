...
def test_validator_rejects_tampered_event_hash(tmp_path):
    mod = load('validator', SCRIPTS / 'validate_agon_mechanical_trial_runs.py')
    source = json.loads((EXAMPLES / 'agon_mechanical_trial_event_log.prediction.example.json').read_text(encoding='utf-8'))
...
def test_simulator_check_green():
    result = subprocess.run([sys.executable, str(SCRIPTS / 'simulate_agon_mechanical_trials.py'), '--check'], cwd=str(ROOT))
