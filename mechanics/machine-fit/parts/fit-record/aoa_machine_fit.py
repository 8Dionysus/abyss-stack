...
SCRIPT_ROOT = find_repo_root(Path(__file__).resolve().parent)
...
def cpu_flags_set(lscpu_data: dict[str, str]) -> set[str]:
    raw_flags = lscpu_data.get("Flags", "")
...
def main() -> int:
    ...
    if drm_nodes["dev_dri_present"] and ai_devices:
        validated_acceleration_posture = (
            "The current reviewed default keeps the canonical llama.cpp text lane on conservative settings. OVMS is the reviewed Intel serving seam currently used for embeddings when explicitly layered, while broader OVMS, OpenVINO, and OpenVINO GenAI lanes may host other model classes only after separate review. Intel NPU is visible but not yet part of the validated canonical path."
        )
    elif drm_nodes["dev_dri_present"]:
        validated_acceleration_posture = (
            "Intel GPU is available for reviewed OVMS and OpenVINO serving experiments when explicitly layered; the canonical text lane still prefers llama.cpp until a separate review promotes something else."
        )
    else:
        validated_acceleration_posture = (
    ...
    if requires_llamacpp_runtime_fallback:
        safety_overlays.append("compose/tuning/llamacpp.runtime-fallback.yml")
    if adaptation_record:
        ...
        raw_overlays = (
            adaptation_record.get("adaptation", {}).get("overlays", [])
            if isinstance(adaptation_record.get("adaptation"), dict)
            else []
        )
            ...
            if preferred_preset == "intel-full":
                for key, value in raw_settings.items():
                    if value is None:
                        continue
                    key_text = str(key)
                    if key_text.startswith("AOA_LLAMACPP_"):
                        validated_settings[key_text] = str(value)
            else:
                validated_settings = {
                    str(key): str(value)
                    for key, value in raw_settings.items()
        ...
        if isinstance(raw_overlays, list) and (
            adaptation_hardware_class is None or adaptation_hardware_class == hardware_class
        ):
            recommended_overlays = [
                str(value)
                for value in raw_overlays
    ...
    if host_facts_ref is None:
        host_facts_ref = (
            ...
            else public_ref("mechanics/machine-fit/parts/host-facts/examples/reference-host.public.json")
            or public_ref("mechanics/machine-fit/parts/host-facts/examples/reference-host.public.json.example")
        )
    ...
    if platform_adaptation_ref is None:
        platform_adaptation_ref = (
            ...
            if args.mode == "private"
            else public_ref("mechanics/machine-fit/parts/platform-adaptations/examples/platform-adaptation.public.json.example")
        )
    ...
    if requires_llamacpp_runtime_fallback:
        envelope_notes.append(
            "CPU flags do not include avx512f; machine-fit should auto-apply compose/tuning/llamacpp.runtime-fallback.yml so llama.cpp stays on the generic server image instead of the crashing server-openvino seam."
        )
    ...
    summary_parts = [
        ...
        "Canonical text should stay on the reviewed llama.cpp path, with OVMS retained as the reviewed Intel serving seam in promoted presets and currently exercised for embeddings while broader Intel-served model lanes stay additive until separately reviewed.",
    ]
    ...
    if recommended_overlays:
        summary_parts.append("A bounded compose overlay is recommended for this host fit.")
    if requires_llamacpp_runtime_fallback:
        summary_parts.append(
            "This host should keep llama.cpp on the generic server image until a reviewed Intel-specific image seam survives live re-check without SIGILL."
        )
    ...
    record = {
        ...
        "runtime_recommendation": {
            ...
            "preferred_runtime_path": "intel-full -> langchain-api /run -> llama.cpp + route-api"
            if preferred_preset == "intel-full"
            else "agent-full -> langchain-api /run -> llama.cpp + route-api",
        ...
        "fit_verdict": {
            ...
            "retest_on": [
                ...
                "llama.cpp, OVMS/OpenVINO serving, or langchain-api runtime change",
